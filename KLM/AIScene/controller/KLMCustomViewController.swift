...
private struct tempColors {
    let maxTemp: Float = 6500
    let minTemp: Float = 2700
}
...
class KLMCustomViewController: UIViewController, Editable {
    ...
    func setupUI() {
        ...
        let colorTempSlider: KLMSlider = KLMSlider.init(frame: CGRect(x: 0, y: 0, width: sliderWidth, height: colorTempBgView.height), minValue: 0, maxValue: 38, step: 1)
        colorTempSlider.indicateViewWidth = 50
        colorTempSlider.getValueTitle = {[weak self] value in
            guard let self = self else { return ""}
            let vv = Int(value)
    ...
    private func setupData() {
        ...
        let parameTime = parameModel(dp: .customerColor)
        KLMSmartNode.sharedInstacnce.readMessage(parameTime, toNode: KLMHomeManager.currentNode)
    }
    ...
    ///发送蓝牙数据
    private func sendBlueToothData() {
        
        let value = currentColor.HSToHexString() + colorTempValue.decimalTo2Hexadecimal() + lightValue.decimalTo2Hexadecimal()
        let parame = parameModel(dp: .customerColor, value: value)
...
extension KLMCustomViewController: KLMSmartNodeDelegate {
    ...
    func smartNode(_ manager: KLMSmartNode, didReceiveVendorMessage message: parameModel?) {
        if message?.dp == .customerColor, message?.opCode == .read, let value = message?.value as? [UInt8] {
            ...
            if value.count >= 6 {
                ...
                //色温
                colorTempValue = Int(value[4])
                //亮度
                lightValue = Int(value[5])
...
extension KLMCustomViewController: RSColorPickerViewDelegate {
    ...
    func colorPicker(_ colorPicker: RSColorPickerView!, touchesEnded touches: Set<AnyHashable>!, with event: UIEvent!) {
        ...
        ///点击色盘
        self.ringSelectView.isHidden = true
        currentColor = colorPicker.selectionColor
        sendBlueToothData()
